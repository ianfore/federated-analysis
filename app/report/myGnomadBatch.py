...
def execute_query(query, symbol, reference_genome, dataset, queryType):
    req_variantlist = {
        "query": query % (symbol, reference_genome, dataset),
        "variables": {}
        }
    response = fetch(req_variantlist)
    return response["data"][queryType]["variants"]
...
def main():
    ...
    # query, symbol, reference_genome, dataset
    x = execute_query(q1, 'ENST00000357654',reference, 'gnomad_' + version, "transcript" )
    print(x)
